...
σ=-(sin(βL[2])+sinh(βL[2]))/(cos(βL[2])+cosh(βL[2]));
x=0:0.01:1;
W=sin.(βL[2]*x)-sinh.(βL[2]*x)+σ*(cos.(βL[2]*x)-cosh.(βL[2]*x))

p2=plot(x,W,lw=2,leg=false,grid=true,dpi=600,xlabel="x/L",ylabel="W2")
display(p2)
